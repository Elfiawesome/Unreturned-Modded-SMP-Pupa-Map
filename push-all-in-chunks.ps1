...
# Works even when there is no commit yet
$branch = (git symbolic-ref --short HEAD 2>$null)
if (-not $branch) {
    Write-Error "Not on a branch (detached HEAD). Checkout a branch first."
    exit 1
}

# Determine remote: prefer branch config, fall back to 'origin', then any remote
...
if (-not $remote) {
    ...
    elseif ($remotes.Count -gt 0)         { $remote = $remotes[0] }
        ...
        Write-Error "No git remote configured. Add one: git remote add origin <url>"
        exit 1
    }
}

# Is there an existing upstream? (only meaningful if we already have commits)
$hasHead = $true
...
if ($hasHead) {
    ...
    if ($LASTEXITCODE -eq 0) { $upstreamSet = $true }
}

Write-Host "Repo:      $repoRoot"
Write-Host "Branch:    $branch (remote: $remote)"
Write-Host "Has HEAD:  $hasHead"
Write-Host "Upstream:  $upstreamSet"
...
$firstPush  = -not $upstreamSet    # first successful push uses -u
...
while ($true) {
    ...
    if ($firstPush) {
        ...
        if ($LASTEXITCODE -eq 0) { $firstPush = $false }
    } else {
